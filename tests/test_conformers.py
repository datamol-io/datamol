...
def test_conformer_energy():
    ...
    assert np.isclose(mol1.GetNumConformers(), 40, atol=1)
    e1 = mol1.GetConformer(1).GetPropsAsDict()
    assert np.isclose(e1['rdkit_UFF_energy'], 35.640740, atol=0.1)
    assert np.isclose(e1['rdkit_UFF_delta_energy'], 0.246822, atol=0.1)

    mol2 = dm.conformers.generate(mol, forcefield='MMFF94s', eratio=3)
    assert np.isclose(mol2.GetNumConformers(), 23, atol=1)
    e2 = mol2.GetConformer(2).GetPropsAsDict()
    assert np.isclose(e2['rdkit_MMFF94s_energy'], 38.715689, atol=0.1)
    assert np.isclose(e2['rdkit_MMFF94s_delta_energy'],  2.220522, atol=0.1)

    mol3 = dm.conformers.generate(mol, forcefield='MMFF94s_noEstat', minimize_energy=True)
    e3 = mol3.GetConformer(3).GetPropsAsDict()
    assert np.isclose(e3['rdkit_MMFF94s_noEstat_energy'], 38.217380, atol=0.1)
    assert np.isclose(e3['rdkit_MMFF94s_noEstat_delta_energy'],  0., atol=0.1)
