...
def df_filter_smiles(
    ...
    for smi in list_of_smi:
        mol = dm.to_mol(smi)
        catalog.append(
            ','.join(get_props(mol,catalog_specifiers = catalog_specifiers))
        )
        des.append(
            ','.join(get_descriptions(mol,catalog_specifiers = catalog_specifiers))
        )
        num_hits.append(
            n_matches(mol,catalog_specifiers = catalog_specifiers)
        )

        

    df = pd.DataFrame(
        data=list(zip(list_of_smi,num_hits,catalog,des)),
        columns=['SMILES','num_of_matches','Catalog','Description']
        )
